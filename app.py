...
def complaintuser():
	sql6="SELECT `cmpid`, `time`, `email`, `title`, `content`, `status`, `reply` FROM `complaints` WHERE status = '0'"
	data=select_records(sql6)
...
def addcandidate():
	...
	if(len(count)>0 and len(cc)<1):
		sql7= "INSERT INTO candidates(cid, admno) VALUES (%s,%s)"
...
def upcomingcampaignindex():
	...
	data=select_records(sql8)	
	return render_template("admin/upcomingcampaign.html",rows=data)
...
def completedcontestindex():
	user=session['user']
	admno=user[2]
	sql8="SELECT cid, name, date, starttime, endtime, status, designation FROM contest WHERE status in (1,2) and date<='"+today+"' and cid in (select cid from electoralroll where admno='"+admno+"' and candidateid !=0) ORDER BY date ASC"
	data1=select_records(sql8)
